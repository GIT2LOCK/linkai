...
class Application:
    ...
    def start(self) -> Any:
        ...
        try:
            if self._is_shell_launcher(launcher_path):
                ...
                # Damos um fôlego inicial para o ClickOnce processar a abertura
                time.sleep(5)
            else:
    ...
    def get_main_window(self) -> WindowSpecification:
        ...
        # Transforma a janela física de volta em uma WindowSpecification 
        # usando o Handle para liberar o método .child_window()
    ...
    def _locate_main_window(self, timeout: float) -> Any:
        ...
        while time.time() < end:
            ...
            for win in windows:
                try:
                    ...
                    print(f"PID: {pid} | Título: '{title}' | Classe: {class_name}")
                    print("=" * 80)
        ...
        print("\n" + "!" * 80)
        print("FALHA AO ENCONTRAR O LUMINA. LISTANDO TODAS AS JANELAS VISÍVEIS NO MOMENTO:")
        for w in desktop.windows(visible_only=True):
            try:
                print(f"PID: {w.process_id()} | Título: '{w.window_text()}' | Classe: '{w.class_name()}'")
            except Exception:
                pass
        print("!" * 80 + "\n")
