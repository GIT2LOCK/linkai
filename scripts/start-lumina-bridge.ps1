...
$Port = if ($env:LINKAI_BRIDGE_PORT) { [int]$env:LINKAI_BRIDGE_PORT } else { 8766 }
...
Set-Location $Root
Write-Output "Iniciando executor Lumina em 0.0.0.0:$Port"
Write-Output "Origens permitidas: $env:LINKAI_ALLOWED_ORIGINS"

& $Python -m uvicorn backend.api.server:app --host 0.0.0.0 --port $Port --log-level debug
